...
# ✅ Dynamic suggestions (no fixed template list)
...
def _friendly_fail(question: str, message: str, plan_kind: str = "error") -> QueryResponse:
    # Broad + non-hardcode: failure driven by parse/validation/execution.
...
def _apply_system_truncation_messaging(narrative: str, explanation: str, meta: dict) -> tuple[str, str]:
    """
    Only for SYSTEM truncation (meta['truncated']).
    This is where we explicitly say "cannot provide full result" and we are showing a preview.
    """
...
def _apply_user_limit_note(explanation: str, meta: dict) -> str:
    """
    For user/plan-level limits (top N, limit_points chosen by plan).
    Normal output: DO NOT add ⚠️ and DO NOT trigger suggestions.
    """
    if not meta.get("user_limited"):
        return explanation
    note = meta.get("user_limit_note") or "Result is limited by the requested top/limit setting."
...
def query(req: QueryRequest):
    ...
    if multi is None:
        try:
            ...
            return _friendly_fail(question, f"Planner failed to produce a valid plan. Error: {e}", plan_kind="planner_error")
        plan_cache.set(plan_key, multi)

    # 2) Validate (schema guardrails)
    ...
    for p in multi.plans:
        ...
        if cached is not None:
            it = ResultItem(**cached)
            items.append(it)
            # cached failure/truncation suggestions are not stored; we rebuild suggestions per request
            # so do nothing here
            continue
        ...
        # Execution failure: add dynamic suggestions + return text step
        if err:
        ...
        # System truncation: add dynamic suggestions
        ...
        # Apply messaging (⚠️ only for system truncation)
        narrative = p.narrative or ""
        narrative, explanation = _apply_system_truncation_messaging(narrative, explanation, meta)
        explanation = _apply_user_limit_note(explanation, meta)
        ...
        # Correlation: default scatter if possible
